...
### RUNNING THE APPLICATION WITH DOCKER COMPOSE FILE
 ## Instructions
- Fork and Clone :  `https://github.com/GBett/Orchestration.git`
- Change in to yolo directory :  `cd Orchestration`
            ...
            spec:
              containers:
                - name: clientcontainer
                  image: gilbert/frontend:1
                  ports:
                    - containerPort: 3000
                    ```

  - This `client.yml` file creates a Pod named `yolo-client` with a single container named `clientcontainer` that runs the `gilbert/frontend:1` image from docker hub and exposes port `3000`.

  - This `api.yml` file creates a Pod named `yolo-api` with a single container named `backendcontainer` that runs the `gilbert/api:1` image from docker hub and exposes port `5000`.
...
```
gilbertbett@cloudshell:~ (shop-375720)$ kubectl get services
